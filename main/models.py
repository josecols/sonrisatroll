...
class Seccion(models.Model):
    ...
    contenido = models.TextField(help_text='Ingrese información relacionada a la sección') 
...
class Publicacion(models.Model):
    ...
    video = models.URLField(help_text='URL del vídeo en YouTube' , blank=True, verbose_name="vídeo")
    ...
    def is_youtube_url(self):
        return re.search(r'^(http|https)\:\/\/www\.youtube\.com\/watch\?v\=(\w*)(\&(.*))?$', self.video)
...
class PerfilUsuario(models.Model):
    ...
    twitter = models.CharField(max_length=15, help_text="Ingresa tu usuario en Twitter sin el @", blank=True)
    facebook = models.URLField(help_text="Ingresa la URL completa de tu perfil en Facebook", blank=True)
    ...
    favoritos = models.ManyToManyField(Publicacion, blank=True, related_name='usuario_favoritos')
    class Meta:
...
def asignar_medallas(sender, instance, **kwargs):
    medalla = None
    try:
        if Publicacion.objects.filter(autor=instance.autor, aprobado=True).count() == 100:
            medalla = Medalla.objects.get(titulo="Sangre de bufón")            
        elif Publicacion.objects.filter(autor=instance.autor, aprobado=True).count() == 1:
...
def usuario_autenticado(sender, user, request, **kwargs):
    dias = (user.last_login - user.date_joined).days
    if dias >= 365:
        try:
            medalla = Medalla.objects.get(titulo="El tiempo vuela cuando te diviertes")            
        except Medalla.DoesNotExist:
            medalla = None
        if medalla:
...
def agregar_etiqueta(sender, instance, action, **kwargs):
    if action == "post_clear" and instance.imagen and gif_animado(instance.imagen.path):
...
m2m_changed.connect(agregar_etiqueta, sender=Publicacion.etiquetas.through)
